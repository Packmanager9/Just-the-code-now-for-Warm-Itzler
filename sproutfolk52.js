...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000055") {
        canvas = canvas_pass
        video_recorder = new CanvasCaptureToWEBM(canvas, 500000);
                                    ...
                                    if (this.fly > 0) {
                                        if ((throbert.road[`${this.xcord},${this.ycord}`].z < throbert.road[`${throbert.t},${throbert.k}`].z) && (throbert.road[`${this.xcord},${this.ycord}`].z <= throbert.road[`${t},${k}`].z)) {
                                            ...
                                            if (Math.abs(throbert.road[`${throbert.t},${throbert.k}`].z - throbert.road[`${t},${k}`].z) > .1 && (throbert.road[`${throbert.t},${throbert.k}`].z < throbert.road[`${t},${k}`].z)) { //Math.abs
                                                this.body.xmom = Math.cos(angle) * (2.14159265 + throbert.magsetoff)
                                                this.body.ymom = Math.sin(angle) * (2.14159265 + throbert.magsetoff)
                                                this.hittime = 1
                                                break
                                            }
                                        }
            ...
            if (this.health > 0) {
                if (this.health < this.maxhealth) {
                    ...
                    if ((this.cycle % 60).between(50, 59)) {
                        this.body.radius = 16 + (Math.abs(5 - (this.cycle % 10)) * 3)
                        this.pulse = 1
                    } else {
                        this.pulse = 0
                        this.body.radius = 16
        ...
        draw() {
            ...
            if (this.health > 0) {
                if (this.health < this.maxhealth) {
                    this.cycle++
                    if ((this.cycle % 60).between(50, 59)) {
                        this.body.radius = 16 + (Math.abs(5 - (this.cycle % ten)) * 3)
                        this.pulse = 1
                    } else {
                        this.pulse = 0
                        this.body.radius = 16
    ...
    class Throbert {
        constructor() {
            ...
            for (let t = 0; t < 1024; t++) {
                for (let k = 0; k < 1024; k++) {
                    ...
                    rect.z = (this.map[`${t * ten},${k * ten}`].z) * 2
                    // console.log(rect.z)
        ...
        generate(num) {
            ...
            for (let t = 0; t < num; t++) {
                let k= t
                t= 0
                const sprout = new Sproutfolk(this.c1.x + (Math.cos(this.angle + ((Math.PI / (4.5)) * (t + sworp))) * (40 + ((t + sworp) * 1.4))), this.c1.y + (Math.sin(this.angle + ((Math.PI / (4.5)) * (t + sworp))) * (40 + ((t + sworp) * 1.4))), k % 3)
                let wet = 1
                t = k
                // while (wet == 1) {
                //     wet = 0
